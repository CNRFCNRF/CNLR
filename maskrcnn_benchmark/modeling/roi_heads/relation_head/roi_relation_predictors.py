...
from maskrcnn_benchmark.modeling.cnlr_head.cross_sub_nodes import CrossNodeGenerate


# from maskrcnn_benchmark.modeling.roi_heads.my_head.head_tail_boxes import GNNLayer
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.dim = 512
        self.cross_node_generate = CrossNodeGenerate(self.dim)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
                cross_head_features, cross_tail_features, cross_boxes_head, cross_boxes_tail,
                cross_matrix, cross_id_matrix, full_id_matrix, logger=None):
        """
        Returns:
            obj_dists (list[Tensor]): logits of object label distribution
            rel_dists (list[Tensor])
        ...
        if self.attribute_on:
            ...
            obj_dists, obj_preds, edge_ctx, obj_embed, _ = self.context_layer(roi_features, proposals, logger)
        ...
        num_rels = [r.shape[0] for r in rel_pair_idxs]
        num_objs = [len(b) for b in proposals]
        id_num = [len(b) for b in cross_id_matrix]
        ...
        cross_node_head_features, cross_node_tail_features = self.cross_node_generate(proposals, cross_head_features,
                                                                                      cross_tail_features,
                                                                                      cross_boxes_head,
                                                                                      cross_boxes_tail,
                                                                                      obj_embed, rel_pair_idxs,
                                                                                      cross_id_matrix)
        cross_node_head_features = self.MLP(cross_node_head_features)
        cross_node_tail_features = self.MLP(cross_node_tail_features)

        cross_node_head_features = cross_node_head_features.split(id_num, dim=0)
        cross_node_tail_features = cross_node_tail_features.split(id_num, dim=0)
        prod_reps = []
        pair_preds = []
        cross_reps = []
        original_reps = []
        u_splits = []
        for pair_idx, head_rep, tail_rep, obj_pred, cross_node_head, cross_node_tail, num_obj, cross_id, full_id, union_split in zip(
                rel_pair_idxs, head_reps, tail_reps, obj_preds, cross_node_head_features, cross_node_tail_features,
                num_objs, cross_id_matrix, full_id_matrix, union_splits):
            zeros = torch.zeros((full_id.shape[0] - cross_id.shape[0]), 512).to(torch.device('cuda'))

            head = torch.cat((cross_node_head, zeros), dim=0)
            tail = torch.cat((cross_node_tail, zeros), dim=0)
            _, original_id = torch.sort(full_id, dim=0)
            head = head[original_id]
            tail = tail[original_id]
            prod_reps.append(torch.cat((head_rep[pair_idx[:, 0]] * (head + 1),
                                        tail_rep[pair_idx[:, 1]] * (tail + 1)), dim=-1))

            head_original_rep = head_rep[pair_idx[:, 0]][cross_id]
            tail_original_rep = tail_rep[pair_idx[:, 1]][cross_id]
            union_split = union_split[cross_id]
            u_splits.append(union_split)
            original_reps.append(torch.cat((head_original_rep, tail_original_rep), dim=-1))
            cross_reps.append(torch.cat((cross_node_head, cross_node_tail), dim=-1))
            pair_preds.append(torch.stack((obj_pred[pair_idx[:, 0]], obj_pred[pair_idx[:, 1]]), dim=1))
        prod_rep = cat(prod_reps, dim=0)
        original_reps = cat(original_reps, dim=0)
        cross_reps = cat(cross_reps, dim=0)

        u_splits = cat(u_splits, dim=0)
        pair_pred = cat(pair_preds, dim=0)
        prod_rep = self.post_cat(prod_rep)
        original_reps = self.post_cat(original_reps)
        cross_reps = self.post_cat(cross_reps)
        if self.use_vision:
            if self.union_single_not_match:
        ...
        original_ctx = self.rel_compress(original_reps * u_splits)
        cross_preds = self.rel_compress(cross_reps)
        ...
        if self.attribute_on:
            ...
            return obj_dists, rel_dists, add_losses, original_ctx, cross_preds
...
class VCTreePredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
        ...
        original_ctx = self.ctx_compress(original_reps * u_splits)
        cross_preds = self.ctx_compress(cross_reps)
...
class CausalAnalysisPredictor(nn.Module):
    ...
    def pair_feature_generate(self, roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger, union_features,
                              cross_head_features, cross_tail_features, cross_boxes_head, cross_boxes_tail,
                              cross_matrix, cross_id_matrix, full_id_matrix,
                              ctx_average=False):
        # encode context infomation
        obj_dists, obj_preds, edge_ctx, obj_embed, binary_preds = self.context_layer(roi_features, proposals,
                                                                                     rel_pair_idxs,
        ...
        u_splits = []
        cross_reps = []
        for pair_idx, head_rep, tail_rep, obj_pred, obj_box, obj_prob, cross_node_head, cross_node_tail, num_obj, cross_id, full_id, union_split in zip(
                rel_pair_idxs, head_reps, tail_reps, obj_preds, obj_boxs, obj_prob_list, cross_node_head_features,
                cross_node_tail_features,
                num_objs, cross_id_matrix, full_id_matrix, union_splits):
            zeros = torch.zeros((full_id.shape[0] - cross_id.shape[0]), 512).to(torch.device('cuda'))
        ...
        return post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list, \
               u_splits, original_reps, cross_reps
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
        ...
        post_ctx_rep, pair_pred, pair_bbox, pair_obj_probs, binary_preds, obj_dist_prob, edge_rep, obj_dist_list, \
        u_splits, original_reps, cross_reps = \
        ...
        if (not self.training) and self.effect_analysis:
            with torch.no_grad():
                avg_post_ctx_rep, _, _, avg_pair_obj_prob, _, _, _, _, u_splits, original_reps, cross_reps = self.pair_feature_generate(
                    roi_features,
                    proposals,
                    rel_pair_idxs,
                    num_objs, obj_boxs,
        ...
        return obj_dist_list, rel_dist_list, add_losses, original_ctx, cross_preds
