...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        refine_logits, relation_logits, add_losses, original_ctx, cross_preds \
            = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,
                             cross_head_features, cross_tail_features, cross_boxes_head, cross_boxes_tail,
                             cross_matrix, cross_id_matrix, full_id_matrix, logger)
        # for test
        if not self.training:
            result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals)
            return roi_features, result, {}

        loss_relation, loss_refine, global_loss_relation, cross_loss_relation = self.loss_evaluator(
            proposals, rel_labels, relation_logits, refine_logits, cross_id_matrix, original_ctx, cross_preds
        )
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            ...
            output_losses = dict(loss_rel=loss_relation,
                                 global_loss=global_loss_relation, cross_loss=cross_loss_relation,
