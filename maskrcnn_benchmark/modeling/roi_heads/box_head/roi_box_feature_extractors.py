...
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals, head_boxes, tail_boxes, cross):
        if cross != True:
            x = self.pooler(x, proposals, head_boxes, tail_boxes, cross)
            x = self.feature(x)
            return x
        else:
            pro_result, head_result, tail_result = self.pooler(x, proposals, head_boxes, tail_boxes, cross)
