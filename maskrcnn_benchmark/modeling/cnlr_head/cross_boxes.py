...
class GenerateCrossBoxes(nn.Module):
    def __init__(self, node_dim):
        super(GenerateCrossBoxes, self).__init__()
        self.node_dim = node_dim
        # self.linear = nn.Linear(self.node_dim, self.node_dim)
    ...
    def forward(self, proposals, rel_pair_idxs):
        ...
        '''横向包含'''
        # longitudinal_matrix = torch.where(cross_judgment_0 <= 0, intersection_matrix, zeros)
        # longitudinal_matrix = torch.where(cross_judgment_1 >= 0, longitudinal_matrix, zeros)
